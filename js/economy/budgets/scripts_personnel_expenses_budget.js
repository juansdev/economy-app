$(function(){
    ...
    $(".generate-personnel-expenses-budget").click(function(){
        ...
        function ResetVariables(){
            ...
            inputPersonnelExpensesBudget=[];

            //Todos los arrays tendran la longitud correspondiente al "numberOfMonths" con un valor diferente a undefined.
            for (let index = 0; index < numberOfMonths; index++) {
                nameOfTheMonth.push("");

                staffExpenses.push(0);

                arrStaffExpenses.push([]);

                //Objetos con evento de escucha a cambios de valores, para los array "Total"
                let objectPersonnelExpensesBudget = {
                    personnelExpensesBudgetInternal: 0,
                    personnelExpensesBudgetListener: function(val) {},
                    set personnelExpensesBudget(val) {
                    this.personnelExpensesBudgetInternal = val;
                    this.personnelExpensesBudgetListener(val);
                    },
                    get personnelExpensesBudget() {
                    return this.personnelExpensesBudgetInternal;
                    },
                    registerListener: function(listener) {
                    this.personnelExpensesBudgetListener = listener;
                    }
                }

                inputPersonnelExpensesBudget.push(objectPersonnelExpensesBudget);
            }
        ...
        function AddValues(){
            ...
            //inputNameOfTheMonth, inputStaffExpenses
            ...
            $(".inputStaffExpenses"+selectMonth).on("keypress",function(e){
                if(e.which == 13 && !$(this).attr("readonly")) {
                    staffExpenses[selectMonth] = parseInt($(this).val());
                    //Juntar todas las ventas en un array
                    arrStaffExpenses[selectMonth].push(staffExpenses[selectMonth]);
                    //Sumar la totalidad de todas las ventas
                    staffExpenses[selectMonth] = 0;
                    for (let index = 0; index < arrStaffExpenses[selectMonth].length; index++) {
                        staffExpenses[selectMonth]+=arrStaffExpenses[selectMonth][index];
                    }
                    //Deshabilitar input
                    $(this).prop("readonly",true);
                    //Calcular la totalidad de ventas totales
                    inputPersonnelExpensesBudget[selectMonth].personnelExpensesBudget =staffExpenses[selectMonth];
                }
            });

            //Ventas totales - INPUT READONLY
            inputPersonnelExpensesBudget[selectMonth].registerListener(
                function(val){
                    $("#inputPersonnelExpensesBudget"+selectMonth).val(inputPersonnelExpensesBudget[selectMonth].personnelExpensesBudget);
                }
            );
        }
        ...
        function UpdateValues(){
            ...
            if($(".inputStaffExpenses"+selectMonth).length){
                for (let index = 0; index < arrStaffExpenses[selectMonth].length; index++) {
                    const value = arrStaffExpenses[selectMonth][index];
                    $(".inputStaffExpenses"+selectMonth).val(value);
                }
            }
        ...
        function UpdateValuesAfterOfDeleted(){
            ...
            //Gastos del personal - INPUT EDITABLE
            if($(".inputStaffExpenses"+selectMonth).length){
                arrStaffExpenses[selectMonth]=[];
                $(".inputStaffExpenses"+selectMonth).each(function(){
                    arrStaffExpenses[selectMonth].push(parseInt($(this).val()));
                });
        ...
        btn_table.click(function(e){
            ...
            if(!idResummaryTable){
                let tables = "<div class=\"my-5\"><Table class=\"table table-info table-striped\"><thead><tr><th colspan=2 className=\"text-center fw-bold\">Presupuesto de gastos del personal</th></tr><tr><th colspan=2 className=\"text-center fw-bold\">"+(productName?"Nombre del producto: "+productName:"")+"</th></tr></thead><tbody>";
                for (let mes = 0; mes < numberOfMonths; mes++) {
                    let pascalCaseSelectTimePeriod = selectTimePeriod[0].toUpperCase()+selectTimePeriod.slice(1).toLowerCase();
                    let element = "<tr><th colspan=2 className=\"text-center fw-bold\">"+pascalCaseSelectTimePeriod+" "+(mes+1)+"</th></tr><tr><td colspan=\"2\" className=\"text-center fw-bold\">"+nameOfTheMonth[mes]+"</td><tr> <td>Gastos del personal</td> <td>"+staffExpenses[mes]+"</td> </tr> <tr> <td>(=) Presupuesto de gastos del personal</td> <td>"+inputPersonnelExpensesBudget[mes].personnelExpensesBudget+"</td> </tr>";
                    tables += element;
                }
                //Tabla totalidad
                let totalStaffExpenses = 0;
                let totalPersonnelExpensesBudget = 0;

                staffExpenses.forEach(element => {totalStaffExpenses+=parseInt(element)});

                inputPersonnelExpensesBudget.forEach(element => {totalPersonnelExpensesBudget+=parseInt(element.personnelExpensesBudget)});
                let element = "<tr><th colspan=2 className=\"text-center fw-bold\">Totalidad</th></tr><tr> <td>Gastos del personal</td> <td>"+totalStaffExpenses+"</td> </tr><tr> <td>(=) Prespuesto de gastos del personal</td> <td>"+totalPersonnelExpensesBudget+"</td> </tr>";
